...
public class MeController implements OnClickListener {
	...
	public void onClick(View v) {
		...
		case R.id.take_photo_iv:
			...
			mDialog = DialogCreator.createSetAvatarDialog(
					mContext.getActivity(), listener);
			mDialog.show();
		...
		case R.id.logout_rl:
			...
			mDialog = DialogCreator.createLogoutDialog(mContext.getActivity(),
					listener);
			mDialog.getWindow().setLayout((int) (0.8 * mWidth),
					WindowManager.LayoutParams.WRAP_CONTENT);
			mDialog.show();
		...
		// this.mDatePickerDialog = new DatePickerDialog(this.getActivity(), new
		// OnDateSetListener() {
		//
		// @Override
		// public void onDateSet(DatePicker arg0, int year, int month, int
		// dayOfMonth) {
