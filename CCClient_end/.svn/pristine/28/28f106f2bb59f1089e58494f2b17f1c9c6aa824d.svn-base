...
public class MainView extends RelativeLayout {
	...
	public void initModule() {
		mBtnListID = new int[] { R.id.actionbar_msg_btn,
		...
		mBtnList[0].setTextColor(getResources().getColor(
				R.color.actionbar_pres_color));
	...
	public void setButtonColor(int index) {
		for (int i = 0; i < 3; i++) {
			if (index == i) {
				...
				mBtnList[i].setTextColor(getResources().getColor(
						R.color.actionbar_pres_color));
			} else {
				mBtnList[i].setSelected(false);
				mBtnList[i].setTextColor(getResources().getColor(
						R.color.action_bar_txt_color));
			}
		}
	}
