...
public class FriendInfoController implements OnClickListener,
	...
	public void onClick(View v) {
		...
		case R.id.friend_info_return_btn:
			...
			mContext.setResult(JChatDemoApplication.RESULT_CODE_FRIEND_INFO,
					intent);
			mContext.finish();
			break;
	...
	public void onChanged(int id, final boolean flag) {
		...
		case R.id.black_list_slip_btn:
			...
			if (flag) {
				JMessageClient.addUsersToBlacklist(list, new BasicCallback() {
					...
					public void gotResult(int status, String desc) {
						dialog.dismiss();
						if (status == 0) {
							Log.d("FriendInfoController",
									"add user to black list success!");
							Toast.makeText(
									mContext,
			...
			break;
		// 滑动免打扰按钮是否加入免打扰,userInfo.setNoDisturb(int flag, BasicCallback
		// callback)
		// flag为1表示加入免打扰,否则为0
		case R.id.no_disturb_slip_btn:
			dialog.show();
			mContext.getUserInfo().setNoDisturb(flag ? 1 : 0,
					new BasicCallback() {
						@Override
						public void gotResult(int status, String desc) {
							dialog.dismiss();
