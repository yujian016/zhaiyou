...
public class GroupSettingController implements OnClickListener {
	...
	private void setTitle(int which) {
		if (which == 1) {
			mGroupSettingView.setTitleText(mContext
					.getString(R.string.group_name_hit));
		...
		if (which == 2) {
			mGroupSettingView.setTitleText(mContext
					.getString(R.string.group_my_name_hit));
			// 初始化EditText
			mGroupSettingView.setEditText(JMessageClient.getMyInfo()
					.getNickname());
