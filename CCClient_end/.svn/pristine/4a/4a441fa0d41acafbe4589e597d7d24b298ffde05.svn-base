...
public class FriendInfoActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mTargetAppKey = getIntent().getStringExtra(
		...
		// 更新一次UserInfo
		final Dialog dialog = DialogCreator.createLoadingDialog(
				FriendInfoActivity.this,
				FriendInfoActivity.this.getString(R.string.loading));
		dialog.show();
		JMessageClient.getUserInfo(mTargetId, mTargetAppKey,
					...
					public void gotResult(int status, String desc,
						...
						if (status == 0) {
							...
							HandleResponseCode.onHandle(
									FriendInfoActivity.this, status, false);
						}
					}
				});
	...
	/**
	 * 如果是从群聊跳转过来，使用startActivity启动聊天界面，如果是单聊跳转过来，setResult然后 finish掉此界面
	 */
	public void startChatActivity() {
		...
		if (conv == null) {
			...
			EventBus.getDefault().post(
					new Event.StringEvent(mTargetId, mTargetAppKey));
		}
		finish();
	}
		...
		if (mUserInfo != null && !TextUtils.isEmpty(mUserInfo.getAvatar())) {
			if (mIsGetAvatar) {
				...
				Bitmap bitmap = NativeImageLoader.getInstance()
						.getBitmapFromMemCache(mUserInfo.getUserName());
				if (bitmap != null) {
					Intent intent = new Intent();
