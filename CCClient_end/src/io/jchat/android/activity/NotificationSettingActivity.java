...
public class NotificationSettingActivity extends BaseActivity implements
	...
	public void onChanged(int id, boolean flag) {
		...
		case R.id.notification_switch:
			if (flag) {
				Toast.makeText(NotificationSettingActivity.this,
						"notification switch is open!", Toast.LENGTH_SHORT)
						.show();
				JMessageClient
						.setNotificationMode(JMessageClient.NOTI_MODE_DEFAULT);
			} else {
				JMessageClient
						.setNotificationMode(JMessageClient.NOTI_MODE_NO_NOTIFICATION);
				Toast.makeText(NotificationSettingActivity.this,
						"notification switch is closed!", Toast.LENGTH_SHORT)
						.show();
			}
			break;
		case R.id.sound_switch:
			if (flag) {
				JMessageClient
						.setNotificationMode(JMessageClient.NOTI_MODE_NO_VIBRATE);
				Toast.makeText(NotificationSettingActivity.this,
						"sound switch is opened!", Toast.LENGTH_SHORT).show();
			} else {
				JMessageClient
						.setNotificationMode(JMessageClient.NOTI_MODE_NO_SOUND);
				Toast.makeText(NotificationSettingActivity.this,
						"sound switch is closed!", Toast.LENGTH_SHORT).show();
			}
			break;
		case R.id.vibrate_switch:
			if (flag) {
				JMessageClient
						.setNotificationMode(JMessageClient.NOTI_MODE_SILENCE);
				Toast.makeText(NotificationSettingActivity.this,
						"vibrate switch is opened!", Toast.LENGTH_SHORT).show();
			} else {
				JMessageClient
						.setNotificationMode(JMessageClient.NOTI_MODE_NO_VIBRATE);
				Toast.makeText(NotificationSettingActivity.this,
						"vibrate switch is closed!", Toast.LENGTH_SHORT).show();
			}
			break;
		}
	}
