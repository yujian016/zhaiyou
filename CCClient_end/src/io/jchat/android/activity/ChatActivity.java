...
public class ChatActivity extends BaseActivity {
		...
		public void onReceive(Context context, Intent data) {
			if (data != null) {
				...
				if (data.getAction().equals(Intent.ACTION_HEADSET_PLUG)) {
					mChatController.getAdapter().setAudioPlayByEarPhone(
							data.getIntExtra("state", 0));
				}
			}
		}
	...
	public void handleMsg(android.os.Message msg) {
		...
		case JChatDemoApplication.REFRESH_GROUP_NAME:
			if (mChatController.getConversation() != null) {
				int num = msg.getData().getInt(
						JChatDemoApplication.MEMBERS_COUNT);
				String groupName = msg.getData().getString(
						JChatDemoApplication.GROUP_NAME);
	...
	private void handleImgRefresh(Intent data) {
		if (mChatController.isGroup()) {
			mChatController.getAdapter().setSendImg(
					data.getIntArrayExtra(JChatDemoApplication.MsgIDs));
	...
	public void onBackPressed() {
		...
		if (mChatController.isGroup()) {
			EventBus.getDefault().post(
					new Event.DraftEvent(mChatController.getGroupId(),
							mChatView.getChatInput()));
		} else {
			EventBus.getDefault().post(
					new Event.DraftEvent(mChatController.getTargetId(),
							mChatController.getTargetAppKey(), mChatView
									.getChatInput()));
		}
	...
	protected void onResume() {
		...
		String targetId = getIntent().getStringExtra(
				JChatDemoApplication.TARGET_ID);
		boolean isGroup = getIntent().getBooleanExtra(
				JChatDemoApplication.IS_GROUP, false);
		if (isGroup) {
			try {
				long groupId = getIntent().getLongExtra(
						JChatDemoApplication.GROUP_ID, 0);
				if (groupId == 0) {
					JMessageClient.enterGroupConversation(Long
							.parseLong(targetId));
				} else
			...
			String appKey = getIntent().getStringExtra(
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == JChatDemoApplication.REQUEST_CODE_TAKE_PHOTO) {
			...
			if (mChatController.isGroup()) {
				GroupInfo groupInfo = (GroupInfo) mChatController
						.getConversation().getTargetInfo();
				UserInfo userInfo = groupInfo.getGroupMemberInfo(JMessageClient
						.getMyInfo().getUserName());
				// 如果自己在群聊中，同时显示群人数
				if (userInfo != null) {
					if (TextUtils.isEmpty(data
							.getStringExtra(JChatDemoApplication.NAME))) {
						mChatView.setChatTitle(this.getString(R.string.group),
								data.getIntExtra(
										JChatDemoApplication.MEMBERS_COUNT, 0));
					} else {
						mChatView.setChatTitle(data
								.getStringExtra(JChatDemoApplication.NAME),
								data.getIntExtra(
										JChatDemoApplication.MEMBERS_COUNT, 0));
					}
			...
			if (!mChatController.isGroup()) {
				String nickname = data
						.getStringExtra(JChatDemoApplication.NICKNAME);
	...
	public void startChatDetailActivity(boolean isGroup, String targetId,
			String appKey, long groupId) {
		Intent intent = new Intent();
		...
		startActivityForResult(intent,
				JChatDemoApplication.REQUEST_CODE_CHAT_DETAIL);
	}
	...
	public void startPickPictureTotalActivity(Intent intent) {
		if (!FileHelper.isSdCardExist()) {
			...
			startActivityForResult(intent,
					JChatDemoApplication.REQUEST_CODE_SELECT_PICTURE);
		}
	}
						...
						runOnUiThread(new Runnable() {
							...
							public void run() {
								...
								if (TextUtils.isEmpty(groupInfo.getGroupName())) {
									mChatView.setChatTitle(ChatActivity.this
		...
		runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				if (msg.getTargetType() == ConversationType.single) {
					...
					// 判断消息是否在当前会话中
					if (!mChatController.isGroup()
							&& targetId.equals(mChatController.getTargetId())
							&& appKey.equals(mChatController.getTargetAppKey())) {
						Message lastMsg = mChatController.getAdapter()
								.getLastMsg();
	...
	private void refreshGroupNum() {
		Conversation conv = JMessageClient.getGroupConversation(mChatController
				.getGroupId());
		...
		if (!TextUtils.isEmpty(groupInfo.getGroupName())) {
			...
			bundle.putString(JChatDemoApplication.GROUP_NAME,
					groupInfo.getGroupName());
			bundle.putInt(JChatDemoApplication.MEMBERS_COUNT, groupInfo
