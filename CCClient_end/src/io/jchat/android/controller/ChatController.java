...
public class ChatController implements OnClickListener, View.OnTouchListener,
		...
		if (mIsGroup) {
			...
			if (fromGroup) {
				mChatView.setChatTitle(mContext.getString(R.string.group),
						intent.getIntExtra(JChatDemoApplication.MEMBERS_COUNT,
								0));
				...
				if (mConv != null) {
					...
					UserInfo userInfo = groupInfo
							.getGroupMemberInfo(JMessageClient.getMyInfo()
									.getUserName());
					// 如果自己在群聊中，聊天标题显示群人数
					if (userInfo != null) {
	...
	public void onClick(View v) {
		...
		case R.id.return_btn:
			...
			if (isGroup()) {
				EventBus.getDefault().post(
						new Event.DraftEvent(getGroupId(), mChatView
								.getChatInput()));
			} else {
				EventBus.getDefault().post(
						new Event.DraftEvent(getTargetId(), mTargetAppKey,
		...
		case R.id.right_btn:
			...
			dismissSoftInput();
			mContext.startChatDetailActivity(mIsGroup, mTargetId,
					mTargetAppKey, mGroupId);
			break;
		// 切换输入
			...
			if (controller != null) {
				...
				case REFRESH_LAST_PAGE:
					...
					if (controller.mChatAdapter.isHasLastPage()) {
						controller.mChatView.getListView().setSelection(
								controller.mChatAdapter.getOffset());
						controller.mChatAdapter.refreshStartPosition();
					} else {
						controller.mChatView.getListView().setSelection(0);
					}
					controller.mChatView.getListView().setOffset(
							controller.mChatAdapter.getOffset());
					break;
				case REFRESH_CHAT_TITLE:
					if (controller.mGroupInfo != null) {
						// 检查自己是否在群组中
						UserInfo info = controller.mGroupInfo
								.getGroupMemberInfo(JMessageClient.getMyInfo()
										.getUserName());
						if (!TextUtils.isEmpty(controller.mGroupInfo
								.getGroupName())) {
							controller.mGroupName = controller.mGroupInfo
									.getGroupName();
							if (info != null) {
								controller.mChatView.setChatTitle(
										controller.mGroupName,
										controller.mGroupInfo.getGroupMembers()
												.size());
								controller.mChatView.showRightBtn();
							} else {
								controller.mChatView
										.setChatTitle(controller.mGroupName);
