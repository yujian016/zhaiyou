...
public class LoginView extends LinearLayout {
	...
	private Boolean invokeIsTestEvn() {
		try {
			Method method = JMessageClient.class
					.getDeclaredMethod("isTestEnvironment");
	...
	public void userNameError(Context context) {
		Toast.makeText(context,
				context.getString(R.string.username_not_null_toast),
				Toast.LENGTH_SHORT).show();
	}

	public void passwordError(Context context) {
		Toast.makeText(context,
				context.getString(R.string.password_not_null_toast),
				Toast.LENGTH_SHORT).show();
	}
	...
	public void isShowReturnBtn(boolean fromSwitch) {
		if (fromSwitch) {
			...
			mReturnBtn.setVisibility(INVISIBLE);
	}
